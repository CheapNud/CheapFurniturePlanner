...
- [ ] (2026-09-04) ci: enter the PLANNER_PG_TEST repo Actions secret (a full Npgsql connection string for a throwaway-capable Postgres server - the smoke test creates and drops its own database on it) so the Postgres provider smoke step in build.yml actually runs instead of skipping [user]
- [x] (2026-08-22 → 2026-09-02) updates security: the velopack feed is plain http on the lan (mitm on the feed = arbitrary code execution); real fixes are tls at the forge (infra item) and package signing upstream in CheapAvaloniaBlazor - revisit when either lands - closed by the public https feed; package signing stays optional upstream [audit]
- [x] (2026-08-21 → 2026-08-22) updates: wire velopack once CheapAvaloniaBlazor ships it (currently 3.2.0, no update api): bump the package, add WithVelopackUpdates() pointed at the (then-lan-only) forge feed to the host builder, inject IUpdateService for a restart chip in the app bar - feed address later moved to the public https forge, see the closed item above [user]
...
- [x] (2026-07-18 → 2026-09-04) multi-user "Mode B" foundation: EF provider swap SQLite → PostgreSQL is in (Database.IsNpgsql() branches every provider-specific mapping, both providers keep their own migration history, the Postgres path runs the same suite via smoke tests); the single-user IOUs are paid (2026-09-04): order/service-ticket-number max+1 generation, trip numbers share the scheme, authoring doc-store load-modify-save windows, DiscountRules unique index, user-admin transactions (last-admin guard TOCTOU + non-atomic user+role insert), unit depart-vs-cancel interleave concurrency token, firm one-default invariant read-then-write TOCTOU, material stock upsert find-or-create race on concurrent receipts/finishes. P2P/offline-sync stays deliberately ruled out (unmergeable ERP invariants). [user]
- [ ] (2026-09-04) multi-user "Mode B" continuation: new CheapFurniturePlanner.Server project reusing the same component/service/domain assemblies with a hardened plain ASP.NET Core host (TLS, no desktop interop or devtools, forwarded headers, login rate limiting) - the desktop app stays a single-user local tool; deploy the server host against Postgres; a SQLite → Postgres import tool for consumers migrating from the desktop app's local data; a work-from-home API phase once the server host is live [user]
