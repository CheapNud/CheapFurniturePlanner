...
- [ ] (2026-07-18) multi-user "Mode B": host the same app on a LAN box (single Kestrel writer, clients via browser); EF provider swap SQLite → Azure SQL / MSSQL / PostgreSQL when concurrency requires; then pay the single-user IOUs (order-number max+1 generation, authoring doc-store load-modify-save windows, DiscountRules unique index). P2P/offline-sync deliberately ruled out (unmergeable ERP invariants). [user]
